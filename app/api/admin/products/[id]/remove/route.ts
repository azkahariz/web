...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await auth.client.rpc("admin_remove_product_references", {
    p_source_product_id: id,
    p_references: parsed.references,
  });
  if (error) return productDependencyRpcError(error, "Penghapusan referensi gagal.");
  const result = data as { status?: string } | null;
  if (result?.status !== "removed") {
    const status = result?.status ?? "invalid_selection";
    return NextResponse.json({ error: productRemoveConflictMessage(status), result }, { status: status === "invalid_selection" ? 400 : 409 });
  }
